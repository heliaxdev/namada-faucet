...
pub fn is_valid_proof_of_work(challenge: &String, solution: &String, difficulty: u64) -> bool {
    ...
    hasher.update(&decoded_challenge).expect("Should be able to hash bytes");
    hasher.update(&decoded_solution).expect("Should be able to hash bytes");
    ...
    for byte in hash_bytes.as_bytes().iter().take(difficulty as usize).cloned() {
        if byte != b'0' {
            return false;
        }
    }
